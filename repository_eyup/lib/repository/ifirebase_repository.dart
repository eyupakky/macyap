...
class FirebaseRepository extends IFirebaseRepository {
  ...
  Future<bool> sendLocation(double locationX, double locationY) async{
    var res = await _dio.post(Constant.baseUrl + Constant.sendLocation,
        data: {"access_token": Constant.accessToken, "location_x": '$locationX',"location_y":'$locationY'}).catchError((onError){
          print(onError);
    });
    if (res.statusCode == 200 && res.data) {
      return Future.value(true);
